...
internal abstract class AbstractTask<I : Input> : Task<I, Output> {
     ...
     * 尝试改变编译状态
     */
    open fun transformCompileStatus(compileStatus: Status, compileResult: Result) = compileStatus
    /**
     * 预期的正常编译状态
     * 正常为返回 Accepted
     */
     ...
     * 改变运行错误的结果
     */
    open fun transformRunError(request: RequestType.Request, expect: Output) = expect
    /**
     * 改变运行成功的结果
     */
    open fun transformRunSuccess(result: Result, expectOutput: Output) = expectOutput
